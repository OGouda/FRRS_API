...
def addNewTable(New_table_name = None):
    # Add new API
    if db.session.query(EventsTable.event_table_name).filter_by(event_table_name=New_table_name).first() is not None:
        ...
        if (New_table_name == None) and (New_table_name == " "): 
            ...
            p = EventsTable(key=key_generated, event_table_name=New_table_name)
